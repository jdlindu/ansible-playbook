...
def filterNone(requestContext, seriesList):
    for idx, series in enumerate(seriesList):
        values = filter(lambda x: x is not None, list(series))
        seriesList[idx] = TimeSeries(series.name, series.start, series.end, series.step, values)
    return seriesList


def summarise(requestContext, seriesList, operate):
    for idx, series in enumerate(seriesList):
        values = None
        if operate == 'max':
            log.info(max(list(series)))
            values = [max(list(series))]
        if operate == 'min':
            values = [min(filter(lambda x: x is not None, list(series)))]
        if operate == 'avg':
            pass
        seriesList[idx] = TimeSeries(series.name, series.start, series.end, series.step, values)
    return seriesList
